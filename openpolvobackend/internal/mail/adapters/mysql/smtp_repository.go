...
func (r SMTPSettingsRepository) GetByUserID(ctx context.Context, userID uuid.UUID) (*domain.SMTPRecord, error) {
	row := r.DB.QueryRowContext(ctx,
		`SELECT user_id, host, port, username, password_enc, from_email, from_name, use_tls, updated_at
	...
	if err := row.Scan(&uid, &host, &port, &user, &passEnc, &fromEmail, &fromName, &useTLS, &updated); err != nil {
...
func (r SMTPSettingsRepository) Upsert(ctx context.Context, s *domain.UserSMTPSettings, passwordEnc []byte) error {
	...
	_, err := r.DB.ExecContext(ctx,
		`INSERT INTO laele_user_smtp_settings (user_id, host, port, username, password_enc, from_email, from_name, use_tls, updated_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
		 ON DUPLICATE KEY UPDATE
		   ...
		   use_tls = VALUES(use_tls), updated_at = VALUES(updated_at)`,
		s.UserID, s.Host, s.Port, s.Username, passwordEnc, s.FromEmail, s.FromName, useTLS, now,
	)
	return err
}
