...
func (s *Store) UpsertDigestSettings(ctx context.Context, d *domain.DigestSettings) error {
	_, err := s.DB.ExecContext(ctx,
		`INSERT INTO laele_user_digest_settings (user_id, timezone, digest_hour, digest_enabled, include_finance_summary, include_tasks, last_digest_sent_on, updated_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?)
		 ON CONFLICT(user_id) DO UPDATE SET timezone = excluded.timezone, digest_hour = excluded.digest_hour, digest_enabled = excluded.digest_enabled,
		 include_finance_summary = excluded.include_finance_summary, include_tasks = excluded.include_tasks,
		 last_digest_sent_on = excluded.last_digest_sent_on, updated_at = excluded.updated_at`,
