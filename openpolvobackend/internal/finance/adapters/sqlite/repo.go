...
func (s *Store) MarkSubscriptionReminderSent(ctx context.Context, subID, userID uuid.UUID, dayUTC time.Time) error {
	d := dayUTC.UTC().Format("2006-01-02")
	_, err := s.DB.ExecContext(ctx,
		`UPDATE laele_finance_subscriptions SET last_reminder_sent_on = ?, reminder_active = 1, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?`,
...
func (s *Store) UpdateDigestLastSentOn(ctx context.Context, userID uuid.UUID, dayUTC time.Time) error {
	d := dayUTC.UTC().Format("2006-01-02")
	_, err := s.DB.ExecContext(ctx,
		`UPDATE laele_user_digest_settings SET last_digest_sent_on = ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?`,
		d, userID.String(),
	)
	return err
}
