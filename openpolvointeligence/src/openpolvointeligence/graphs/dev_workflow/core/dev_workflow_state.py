...
class RequirementsOutput(TypedDict, total=False):
...
class ImpactReport(TypedDict, total=False):
...
class ProjectContext(TypedDict, total=False):
    """Contexto carregado via terminal antes do workflow."""
...
class DeliveryGateResult(TypedDict, total=False):
    passed: bool
    failures: list[str]
    checks_run: list[str]
...
class DevWorkflowState(TypedDict, total=False):
    ...
    terminal_mode: Literal["bridge", "memory", "sandbox"]
    terminal_errors: list[dict[str, Any]]
    project_context: ProjectContext
    type_check_result: dict[str, Any]
