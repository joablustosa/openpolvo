 ...
 * instantâneo ao regressar ao mesmo projecto (padrão Bolt.new / Lovable).
 */
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import { AlertTriangle, ExternalLink, FileCode2, RefreshCw, RotateCcw } from "lucide-react";
...
function phaseLabel(phase: ContainerPhase): string {
  switch (phase) {
    case "idle":
      return "A iniciar…";
    case "booting":
      return "A arrancar WebContainer…";
    case "mounting":
      return "A montar ficheiros…";
    case "installing":
      return "A instalar dependências (npm)…";
    case "starting":
      return "A iniciar servidor dev…";
    case "ready":
...
function phaseDescription(phase: ContainerPhase): string {
  switch (phase) {
    case "idle":
    case "booting":
      return "A arrancar o ambiente WebContainer…";
    case "mounting":
      return "A montar ficheiros do projecto…";
    case "installing":
      return "A instalar dependências (npm install). Na primeira execução pode demorar vários minutos…";
    case "starting":
      return "A iniciar o servidor Vite/Next e a aguardar o URL de preview…";
    default:
      return "A preparar…";
  }
}
...
export function BuilderWebContainerPreview({ data }: Props) {
  ...
  const appendLog = useCallback((chunk: string) => {
    setLog((prev) => (prev + chunk).slice(-24_000));
  }, []);
  ...
  useEffect(() => {
    ...
    // Tentar restauro instantâneo (mesmo projecto, servidor já activo)
    if (!mgr.tryFastRestore(key, files)) {
      void mgr.init(files, key);
    }

    return () => {
      // Desanexa callbacks — NÃO mata o servidor dev (persiste para restauro rápido)
      mgr.detach();
  ...
  return (
    <div className="flex h-full min-h-[min(52vh,520px)] w-full min-w-0 flex-1 flex-col gap-0 bg-background">
      ...
      {/* Conteúdo principal */}
      <div className="grid min-h-0 flex-1 grid-cols-1 gap-0 border-b border-border md:grid-cols-[minmax(180px,240px)_1fr]">
        {/* Coluna de ficheiros */}
      ...
      (phase === "installing" ||
        ...
        phase === "ready" ||
        phase === "error") ? (
        <details className="shrink-0 border-t border-border bg-card/30" open={phase === "error"}>
