...
import { OpenPolvoAgent } from './openpolvo/openPolvoAgent.js';
...
import { isOpenPolvoAgentEnabledFromEnv } from '../common/openpolvoConfiguration.js';
...
async function main(): Promise<void> {
	...
	if (!options.quiet) {
		...
		if (isOpenPolvoAgentEnabledFromEnv()) {
			const openPolvoAgent = disposables.add(instantiationService.createInstance(OpenPolvoAgent));
			agentService.registerProvider(openPolvoAgent);
			log('OpenPolvoAgent registered');
		}
