...
class SessionsSetUpWidget extends Disposable {
	...
	private async _watchSignInState(): Promise<void> {
		...
		this.onCompleted();
		this.watcherRef.value = this._watchActiveState(!!initialAccount);
	}
	...
	private _watchActiveState(signedIn: boolean): IDisposable {
		...
		disposables.add(this.defaultAccountService.onDidChangeDefaultAccount(account => {
			...
			if (signedIn && !nowSignedIn && !openPolvoSignedIn) {
				this.storageService.remove(WELCOME_COMPLETE_KEY, StorageScope.APPLICATION);
				this._showWelcome(false);
			}
		...
		disposables.add(this.configurationService.onDidChangeConfiguration(e => {
			if (e.affectsConfiguration(OpenPolvoApiTokenSettingId) || e.affectsConfiguration(OpenPolvoAgentEnabledSettingId)) {
				...
				openPolvoSignedIn = nowOpenPolvoSignedIn;
			}
	...
	private async _showSignInDialog(): Promise<void> {
		...
		const setupOptions = {
			...
			disableCloseButton: true,
			...(openPolvoEnabled ? { setupStrategy: ChatSetupStrategy.SetupWithOpenPolvo } : {}),
		};

		const signingInDialogRef = new MutableDisposable<DisposableStore>();

		const success = await this.commandService.executeCommand<boolean>('workbench.action.chat.triggerSetup', undefined, openPolvoEnabled ? setupOptions : {
			...setupOptions,
			onSignInStarted: () => {
				const disposables = new DisposableStore();
