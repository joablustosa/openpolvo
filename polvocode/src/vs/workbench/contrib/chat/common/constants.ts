...
export function getDefaultNewChatSessionType(
	...
	if (defaultType === 'agent-host-openpolvo' && !isEditorLocalAgentEnabled(configurationService)) {
		return defaultType;
	}
...
export function isVisibleEditorChatSessionType(
	...
	if (sessionType === localChatSessionType) {
		const defaultProvider = configurationService.getValue<string>(ChatConfiguration.EditorDefaultProvider);
		if (!isEditorLocalAgentEnabled(configurationService) && (defaultProvider === 'copilotAh' || defaultProvider === 'openpolvoAh')) {
			return false;
		}
