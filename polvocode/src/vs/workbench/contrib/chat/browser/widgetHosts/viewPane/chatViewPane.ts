...
export class ChatViewPane extends ViewPane implements IViewWelcomeDelegate {
	...
	private shouldSkipRestoredLocalSession(sessionResource: URI, model: IChatModel): boolean {
		...
		const prefersAgentHostOpenPolvo = this.configurationService.getValue<boolean>(ChatConfiguration.EditorLocalAgentEnabled) === false
			&& this.configurationService.getValue<string>(ChatConfiguration.EditorDefaultProvider) === 'openpolvoAh';
		return (defaultType !== localChatSessionType || prefersAgentHostCopilot || prefersAgentHostOpenPolvo)
